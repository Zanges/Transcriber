...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hotkey_handler = Arc::new(HotkeyHandler::new(&config.hotkey, Some(audio_recorder.clone()), Some(openai_transcriber.clone()), Some(output_handler.clone()))?);
    ...
    tokio::spawn(async move {
        hotkey_handler_clone.handle_events(rx).await;
    });
